...
func (s *PetDaycareServiceImpl) GetPetDaycare(id uint, lat *float64, long *float64) (*model.GetPetDaycareDetailResponse, error) {
	...
	if lat != nil && long != nil {
		if err := s.db.
			...
			Preload("Slots.PetCategory").
			Preload("Slots.PetCategory.SizeCategory").
			Select("*, ST_DistanceSphere(ST_MakePoint(?, ?), ST_MakePoint(pet_daycares.longitude, pet_daycares.latitude))/1000 AS Distance", *long, *lat).
...
func (s *PetDaycareServiceImpl) UpdatePetDaycare(id uint, userId uint, newData model.UpdatePetDaycareRequest) (*model.PetDaycareDTO, error) {
	...
	// if err := tx.Unscoped().Where("daycare_id = ?", daycare.ID).Delete(&model.Thumbnail{}).Error; err != nil {
	// 	tx.Rollback()
	// 	return nil, err
	// }
	...
	if len(newData.ThumbnailURLs) > 0 {
		if err := tx.Debug().Where("daycare_id = ?", daycare.ID).Delete(&model.Thumbnail{}).Error; err != nil {
			tx.Rollback()
			return nil, err
		}

		for _, url := range newData.ThumbnailURLs {
			// if err := tx.Debug().Create(&model.Thumbnail{DaycareID: daycare.ID, ImageUrl: url}).Error; err != nil {
			// 	tx.Rollback()
			// 	return nil, err
			// }
			thumbnails = append(thumbnails, model.Thumbnail{DaycareID: daycare.ID, ImageUrl: url})
		}
	...
	if len(newData.PetCategoryID) > 0 && len(newData.MaxNumber) > 0 {
		...
		for i := range newData.PetCategoryID {
			slots = append(slots, model.Slots{
				...
				PricingTypeID: newData.PricingType,
			})
		}
	...
	// Save updates
	if err := tx.Debug().Save(&daycare).Error; err != nil {
		tx.Rollback()
		return nil, err
	}
...
func (s *PetDaycareServiceImpl) GetPetDaycares(req model.GetPetDaycaresRequest, page int, pageSize int) (model.ListData[model.GetPetDaycaresResponse], error) {
	...
	query := s.db.Debug().Table("pet_daycares").
		Select("pet_daycares.id, pet_daycares.name, pet_daycares.locality, ST_DistanceSphere(ST_MakePoint(?, ?), ST_MakePoint(pet_daycares.longitude, pet_daycares.latitude))/1000 as distance, pet_daycares.booked_num", longitude, latitude)
